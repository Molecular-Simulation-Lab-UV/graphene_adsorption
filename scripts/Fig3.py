...
#table = "/home/mbarria/don-elias/AA_GO0US/tables/2022_02_07_simple_diff/Table1.csv"
table = "/home/mbarria/don-elias/nas/AA_GO0US/tables/2022_04_05_parallel_asterisk/Table1.csv"
#output
output = "../figures"
imagename = "Fig3_test"
...
# Testing supposedly better values
sasa_dict = {
    "Alanine"          : -1.02,
    "Arginine"         : -1.50,
    "Arginine_H+"      : -1.60,
    "Asparagine"       : -1.20,
    "Aspartic_Acid"    : -1.08,
    "Aspartic_Acid_H+" : -1.16,
    "Cysteine_H+"      : -1.11,
    "Glutamic_Acid"    : -1.13,
#    "Glutamic_Acid*"    : -1.14,
    "Glutamic_Acid_H+" : -1.34,
    "Glutamine"        : -1.41,
    "Glycine"          : -1.01,
    "Histidine_2H+"    : -1.38,
    "Histidine_H+_ND1" : -1.36,
...
fig, axs = plt.subplots(figsize=(7, 15), ncols=1, nrows=3)
...
for aa in sorted_aa.keys():
    ...
    # Middle by weight
    axs[1].plot([weight], [values[0] - correction_A], color=color, marker=f"${letter}$", markersize=size2)
    # Bottom by SASA
    axs[2].plot([sasa], [values[0] - correction_A], color=color, marker=f"${letter}$", markersize=size2)
...
#axs[2].set_yticklabels(["-25", "-20", "-15", "-10", "-5", "0", "5"], fontsize=13)
#axs[2].set_yticks([-25, -20, -15, -10, -5, 0, 5])
axs[2].set_ylim(-25, 5)
...
axs[1].set_xlabel("Molecular Weight $[g \cdot mol^{-1}]$", fontsize=14)
#axs[1].set_xticklabels(["70", "90", "110", "130", "150", "170", "190", "210"], fontsize=13)
#axs[1].set_xticks([70, 90, 110, 130, 150, 170, 190, 210])
axs[1].set_xlim(70, 210)
leg2_colors = axs[1].legend(handles=custom_handles3, loc="lower left", frameon=False, fontsize=12)

# Bottom ax

axs[2].set_xlabel(r"$\Delta$SASA$^{ads}$ $[nm^{2}]$", fontsize=14)
#axs[2].set_xticklabels(["-1.8", "-1.6", "-1.4", "-1.2", "-1.0", "-0.8"], fontsize=13)
#axs[2].set_xticks([-1.8, -1.6, -1.4, -1.2, -1.0, -0.8])
axs[2].set_xlim(-1.8, -0.9)
leg3_colors = axs[2].legend(handles=custom_handles4, loc="upper left", frameon=False, fontsize=12)
...
# middle ax
lr_weight = linregress(weights, free_energies)
print(f"Prediction of water by weight: {lr_weight.slope * -0.76 + lr_weight.intercept}")

w = np.linspace(0, 210, 50)
axs[1].plot(w, w*lr_weight.slope + lr_weight.intercept, color="k",
#               label=f"{lr.slope:.1f}m + {lr.intercept:.1f}, $r^2$={lr.rvalue**2:.2f}",
            label=f"$r^2_{{neutral}}={lr_weight.rvalue**2:.2f}$",
            linewidth=1, linestyle="--")
leg2 = axs[1].legend(loc="upper right", frameon=False, fontsize=14)
axs[1].add_artist(leg2)
axs[1].add_artist(leg2_colors)

lr_sasa = linregress(sasas, free_energies)
print(f"Prediction of water by weight: {lr_sasa.slope * -0.76 + lr_sasa.intercept}")

w = np.linspace(-1.8, -0.7, 50)
axs[2].plot(w, w*lr_sasa.slope + lr_sasa.intercept, color="k",
            #label=f"{lr_sasa.slope:.1f}m + {lr_sasa.intercept:.1f}, $r^2$={lr_sasa.rvalue**2:.2f}",
            label=f"$r^2_{{neutral}}={lr_sasa.rvalue**2:.2f}$",
            linewidth=1, linestyle="--")
leg3 = axs[2].legend(loc="lower right", frameon=False, fontsize=14)
axs[2].add_artist(leg3)
axs[2].add_artist(leg3_colors)
