...
class Simulation():
    def __init__(self, name, kind, sequence, charge, ff = ff, ifp = ifp, mtb = mtb, seed='0', graphene = False, prepDir = prepDir, outputDir = outputDir, templateDir = templateDir, frameoutDir = frameoutDir, leftraruDir = leftraruDir, gromos = gromos, gromosJAG = gromosJAG, vmd = vmd, md = md, md_leftraru = md_leftraru, local_elevation = False, umbrella_sampling = False, restrain_graphene = True, genbox = genbox, aaUser = False, pepUser = False, sidechain = True, w0_dict = w0_dict, stop = 100, LINCS = False, block = 'CCC', percentage=0):
        ...
        self.frameout = lambda destination, cnf, charge = False, solvent = False, run = True, top = False, graphene = False, gather = False, move = True, frameoutDir = self.frameoutDir : Simulation.frameout(self.name, self.kind, destination, cnf, self.prepDir, self.ff, charge, solvent, self.gromos, run, top, graphene, gather, move, frameoutDir)
        ...
        self.fix_graphene_top = lambda x = 0: Simulation.fix_graphene_top(name = self.name, top = 'graphene_%s_%s.top' % (self.ff, self.kind), coord = 'graphene_%s.cnf' % self.kind, directory = self.prepDir)
        self.make_box = lambda cnf = self.cnf_min, below = '1.500000000', SSD0 = '0.300000000', graphene_min=False : Simulation.make_box(self.name,self.kind, cnf, self.prepDir, False, self.ff, self.gromos, True, below, SSD0, graphene_min)
        self.graphene_com_top = lambda top = self.top, graph_top = False : Simulation.graphene_com_top(self.name, self.kind, top, self.prepDir, False, False, self.frameoutDir, self.ff, self.gromos, graph_top)
        ...
        self.leftraru_jobs = lambda cores = 8, job = 'md' , r = False: Simulation.leftraru_jobs(self.name, self.kind, self.templateDir, self.leftraruDir, self.outputDir, cores, self.pepUser, self.aaUser, self.graphene, job, r, stop = self.stop)
    ...
    def frameout(name, kind, destination, cnf, directory = prepDir, ff = ff, charge = False, solvent = False, gromos = gromos, run = True, top = False, graphene = False, gather = False, move = True, frameoutDir = frameoutDir):
        """
...
#                    imd.write('END\n')
                ...
                if not restrain_graphene:
                    ...
                    for line in coord:
                        if 'POSITION' in line and 'REF' not in line:                
                            ...
                            pass
                        elif write_flag and 'CCC' in line: # and any([(x in line) for x in ['C1', 'C2', 'C3', 'C4']]):
                            por.write(line)    
        with open(directory + name + '/%s' % cnf,'r') as coord:                    
    ...
    def umbrella_sampling_file(name,kind, cnf, r0, directory = outputDir, w0_dict = w0_dict):
        ...
        with open(directory + name + '/%s/%s' % (kind,dsr),'w') as dsr:
            ...
            dsr.write("\n %s 0 0 0 0\t %s -2\t %1.3f %i 60\n" % (Simulation.find_alpha_carbon(directory + name + '/' + cnf, kind),Simulation.find_ccc_corners(directory + name + '/' + cnf), r0, w0_dict[round(r0,4)]))
            dsr.write("END\n")      
    ...
    def find_ccc_corners(cnf):
        ...
        with open(cnf,'r') as file:
            ccc = False
            for line in file:
                if ccc and 'SOLV' in line:
                    break
                elif 'CCC' in line:
                    ccc = True
                    n, x, y = line.split()[3:6]
                    if any(C in line for C in ['C1', 'C2', 'C3', 'C4']):
                        px.append(float(x))
                        py.append(float(y))
                        num.append(int(n))
                    else: # Add false values for O and H, but make sure they are not extremes
                        px.append(2.5)
                        py.append(2.5)
                    # num.append(int(n))
        ...
        return ' '.join([ str(x) for x in (np.asarray(num[:4]) + num[0] - 1)])
    ...
    def make_box(name, kind, cnf = False, directory = prepDir, charge = False, ff = ff, gromos = gromos, rotate = True, below = '1.500000000', SSD0 = '0.300000000', graphene_min=False):  
        if not cnf:
        ...
        #Read graphene
        if not graphene_min:
            graphene_min = 'graphene_%s_min.cnf' % kind
        with open(directory + name + '/' + graphene_min, 'r') as file:
            pos = False
            lattice = False
            genbox = False
            for line in file:
        ...
        Simulation.frameout(name, kind, 'TEMP/', cnf , move = False, directory=directory)
    ...
    #    new_coords += displacement
        ...
        for i in graphene_pos[1:]:
            line = re.split(r'(\s+)', i) 
            #line[14] = below
            line[14] = '%1.9f' % (float(line[14]) + float(below))
            line[1] = line[1][:-1]
            line[2] = '10'
    ...
    def graphene_com_top(name, kind, top = False, directory = prepDir, charge = False, solvent = False, frameoutDir = frameoutDir, ff = ff, gromos = gromos, graph_top = False):
        #Topology
        if not top:
            top = Simulation.get_top(name,kind,ff,charge)
        file = 'com_top_%s_graphene.arg' % kind
        if not graph_top:
            graph_top = 'graphene_%s_%s.top' % (ff, kind)
        with open(directory + name + '/' + file, 'w') as arg:  
            arg.write('@topo\t%s\t%s\n' % (top, graph_top))
            arg.write('@param\t1\n')
            arg.write('@solv\t1\n')
    ...
    def prepare(self):
        ...
        if self.graphene:
            ...
            if not self.percentage:
                ...
                self.graphene_top = f'../../Bases/graphene_fixed_{self.percentage}%.top'
                self.graphene_cnf = f'../../Bases/graphene_ox{self.percentage}%.cnf'
                self.graphene_cnf_min = f'../../Bases/graphene_ox{self.percentage}%_min.cnf'
                self.graphite_top = f'../../Bases/graphene_fixed_{self.percentage}%.top'
              
            #Solvate
            self.make_box(graphene_min=self.graphene_cnf_min)
            self.graphene_com_top(graph_top = self.graphite_top)
        ...
        if self.local_elevation:
            ...
            for r in sorted(self.w0_dict.keys()): # np.arange(0.3,1.50001,0.05):
                ...
                self.leftraru_mk_script(job = 'US', r = r)
                self.leftraru_jobs(job = 'US', r = r)
                self.umbrella_sampling_file(r, cnf=self.cnf_graph_charged_solvent)
