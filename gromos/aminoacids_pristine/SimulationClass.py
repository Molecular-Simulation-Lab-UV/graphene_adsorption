...
leftraruDir = '/home/username/md_h2o_100NS_54a8_3_neutral/'
...
class Simulation():
    def __init__(self, name, kind, sequence, charge, ff = ff, ifp = ifp, mtb = mtb, seed='0', graphene = False, prepDir = prepDir, outputDir = outputDir, templateDir = templateDir, frameoutDir = frameoutDir, leftraruDir = leftraruDir, gromos = gromos, gromosJAG = gromosJAG, vmd = vmd, md = md, md_leftraru = md_leftraru, local_elevation = False, umbrella_sampling = False, restrain_graphene = True, genbox = genbox, aaUser = False, pepUser = False, sidechain = True, w0_dict = w0_dict, stop = 100):
        ...
        self.make_graphene = lambda x = 0 : Simulation.make_graphene(self.name, self.kind, self.cnf_solvent, self.prepDir, self.templateDir, self.ff, self.gromos, self.genbox)
        self.graphite = lambda lastgraph, top = 'graphene_separate_%s_%s.top' % (self.ff, self.kind), cnf = 'graphene_%s.cnf' % self.kind : Simulation.graphite(self.name, self.kind, top, cnf, self.prepDir, False, True, self.ff, self.gromos, self.gromosJAG, lastgraph)
...
#                    imd.write('END\n')
    ...
    def make_graphene(name, kind, cnf, directory = prepDir, templates = templateDir, ff = ff, gromos = gromos, genbox = False):
    ...
    def graphite(name, kind, top, cnf = False, directory = prepDir, charge = False, solvent = True, ff = ff, gromos = gromos, gromosJAG = gromosJAG, lastgraph = '1'):
    ...
    def prepare(self):
        ...
        if self.local_elevation:
            ...
            for r in np.arange(0.3,1.50001,0.05):
                ...
                self.make_imd(self.cnf_graph_charged_solvent, 'US.imd', self.outputDir, self.outputDir, TAU = '0.01', solvent = True, sidechain = self.sidechain, graphene = self.graphene, charge = self.charge, top = self.top_graph_charged, add_restraint = self.restrain_graphene, r = r)
            ...
            self.make_imd(self.cnf_graph_charged_solvent, 'md.imd', self.outputDir, self.outputDir, TAU = '0.01', solvent = True, sidechain = self.sidechain, graphene = self.graphene, charge = self.charge, top = self.top_graph_charged, add_restraint = self.restrain_graphene)
